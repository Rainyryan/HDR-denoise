...
if __name__ == "__main__":
    ...
    model = DualSNRDenoiser(dim=32, num_blocks=[4,4,4,4]).to(device)
    ...
    if os.path.exists(pretrained_path):
        print("Loading pretrained single model into both experts")
        checkpoint = torch.load(pretrained_path)
        # Load weights into both experts to give them a good starting point
        model.denoiser_low_snr.load_state_dict(checkpoint['model_state_dict'], strict=False)
        model.denoiser_high_snr.load_state_dict(checkpoint['model_state_dict'], strict=False)
        print("Initialized both experts with pretrained weights.")
        
    if os.path.exists(save_path):
        print("Loading continued training dual model")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        while not improved:
            ...
            for i in range(len(dataset)):
                ...
                # Fetch the SNR map calculated by HDR_dataset_snr_map
                snr_map = sample["snr_map"].to(device, non_blocking=True)

                optimizer.zero_grad(set_to_none=True)
                
                with torch.cuda.amp.autocast():
                    # Forward pass through the Dual Model
                    y_pred, y_low_snr, y_high_snr = model(xm, snr_map)
                    
                    gt_tm = hdr_tonemap(y, nbits)
                    
                    # 1. Main Blended L2 Loss
                    loss_l2_main = torch.mean((hdr_tonemap(y_pred, nbits) - gt_tm) ** 2)
                    
                    # 2. Auxiliary L2 Losses to force specialization
                    # Threshold SNR at 0.5 to define strictly "low" vs "high" regions
                    mask_low = (snr_map < 0.5).float()
                    mask_high = (snr_map >= 0.5).float()
                    
                    # MSE only on the specific regions they handle
                    loss_aux_low = torch.sum(mask_low * (hdr_tonemap(y_low_snr, nbits) - gt_tm)**2) / (torch.sum(mask_low) + 1e-6)
                    loss_aux_high = torch.sum(mask_high * (hdr_tonemap(y_high_snr, nbits) - gt_tm)**2) / (torch.sum(mask_high) + 1e-6)
                    
                    # 3. Perceptual LPIPS Loss on the final blended output
                    ...
                    aux_weight = 0.5 # Weight for auxiliary specialization losses
            ...
            for i in range(min(5, batch_sz)):
                # Save out GT, Pred, Noisy, and SNR map for visualization
                ...
                # SNR Map is 1-channel, save as grayscale
                image_snr = snr_map[i].detach().cpu().numpy()
                image_snr = np.transpose(image_snr, [1,2,0]) * 255.0
                cv2.imwrite(f"{output_dir}{i}np_snr_map.png", image_snr.astype(np.uint8))

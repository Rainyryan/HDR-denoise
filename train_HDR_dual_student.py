...
if __name__ == "__main__":
    ...
    # 1. Stage 3: Expanded Mode (Latest stage)
    if os.path.exists(save_path) or os.path.exists(best_save_path):
        ...
        # Prefer the standard save over the best save to resume from the exact last epoch
    ...
    # 2. Stage 2: Pre-expand Mode (Middle stage)
    ...
    # 3. Stage 1: Pretrained Student Initialization (Start)
    elif os.path.exists(pretrained_path):
        print("Loading pretrained single student model into both experts")
        checkpoint = torch.load(pretrained_path)
        
        # Check if the state dict comes from the distiller or standard model
        ...
        while not improved:
            ...
            for i in range(len(dataset)):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # 2. Auxiliary L2 Losses to force specialization (Fixed Channel Broadcasting)
                    ...
                    # Multiplying denominator by 3.0 to match the 3 channels in se_low/se_high
                    ...
                    aux_weight = 0.1  # Lowered slightly from 0.5 for stable convergence

...
class HDR_model(nn.Module):
	def __init__(self, 
		out_channels=3, 
		dim = 32, # feature dimension for hidden layers
		num_blocks = [4,4,4,4], #[4,6,6,8], number of Swin blocks at each level
		num_refinement_blocks = 4, # full resolution refinement blocks
		heads = [1,2,4,8],
		...
		# ======== Encoder ========
		self.encoder_level_1 = nn.Sequential(*[SwinTransformerBlock(dim=dim, num_heads=heads[0], window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2) 
		...
		self.encoder_level_2 = nn.Sequential(*[SwinTransformerBlock(dim=int(dim*4), num_heads=heads[2], window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2) 
		...
		# ======== Latent ========
		self.latent		 = nn.Sequential(*[SwinTransformerBlock(dim=int(dim*16), num_heads=heads[3], window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2) 
		...
		## Level 2
		self.decoder_level_2 = nn.Sequential(*[SwinTransformerBlock(dim=int(dim*8), num_heads=heads[2], window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2)  
		...
		## Level 1
		self.decoder_level_1 = nn.Sequential(*[SwinTransformerBlock(dim=int(dim*2), num_heads=heads[1], window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2) 
			for i in range(num_blocks[1])])			## OUT: dim*2
		
		# ======== Final blocks ========
		self.decoder_level_0 = nn.Sequential(*[SwinTransformerBlock(dim=int(dim*2), num_heads=heads[0], window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2) 
			for i in range(num_refinement_blocks)])	## OUT: dim*2
		self.up_shuffle_1_0 = nn.PixelShuffle(2)  ## OUT: dim/2

		# ======== Final full-res blocks ========
		self.refinement_trans = nn.Sequential(*[SwinTransformerBlock(dim=int(dim//2), num_heads=heads[0], window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2) 
